		...
		objects : {
			...
			lumise : {
				...
				'image' : function(ops, callback) {
					...
					$('#LumiseDesign').attr({'data-processing': 'true', 'data-msg': '请稍候..'});
										...
										xhr.upload.addEventListener("progress", function(evt){
											...
											if (evt.lengthComputable) {
												...
												// $('div#LumiseDesign').attr({'data-msg': parseInt(percentComplete*100)+'% upload complete'});
													$('div#LumiseDesign').attr({'data-msg': parseInt( '下载完成'+percentComplete*100)+'%'});
												else{
		...
		render : {
			...
			colorPresets : function() {
				if(lumise.data.status==1) {
					window.location.href = "http://diy.cmygx.cn/index.php?s=/store/index/index";exit;
				}
				...
				lumise.indexed.list(function(data){
					...
					if (st == 'done') {
						...
						if (is_save) {
							...
							// alert(3)
							$('#lumise-saved-designs').append('<p class="empty">暂无内容!</p>');
						}
					}
				});
...
// alert(3)
				...
				lis += '<li data-id="'+design.id+'" data-use-text="'+lumise.i(212)+'" data-func="edit" data-save-text="'+lumise.i(213)+'" data-created="'+design.created+'" data-name="'+design.name+'" class="bgcolorafter">\
							...
							<span data-view="name" data-id="'+design.id+'" data-func="name" title="'+lumise.i(52)+'" data-enter="blur" contenteditable>'+(design.name ? design.name : '未命名')+'</span>\
							<em data-view="date">加入时间：'+lumise.fn.date('Y-M-d h:m', design.updated*1000)+'</em>\
...
// alert(4)
			...
			list_products : function(res) {
				...
				var cates = ['<ul data-view="categories" style="display: none">',
						'<h3>'+lumise.i(56)+'</h3>',
				...
				if (res.products && res.products.length > 0) {
					...
					res.products.map(function(p) {
						...
						if(p.xy_id > 303) {
							p_img = 'http://qnimg.weiladiy.com/'+p.product_logo;
						}
						prods.push(
							'<li data-id="'+p.id+'"'+(
								(lumise.data.product == p.id) ? ' data-current="true"':''
							)+' data-name="'+
							p.product_name.toLowerCase().trim().replace(/[^a-z0-9 ]/gmi, "")+
							'"'+((p.id !== p.product)? ' data-cms="'+p.product+'"': '')+'>\
								<span data-view="thumbn" data-start="'+btn_text+'">\
									<img '+template+' style="background:'+color+'" src="'+(p_img)+'" />\
								</span>\
								<span data-view="name">'+p.product_name+'</span>\
								<span data-view="price">'+lumise.fn.price(p.price_min)+'</span>\
							</li>'
						)
					});
...
// console.log(1)
			...
			process_add_cart	: function(cart_design) {
				...
				var cart_data1		= JSON.parse(localStorage.getItem('LUMISE-CART-DATA') || '{}');
    ...
    //                         design_id:p_id,
			...
			checkout			: function(data) {
				...
				if (lumise.data.max_upload_size > 0 && upload_size/1024000 > lumise.data.max_upload_size) {
					lumise.fn.notice('错误: 您的设计稿超过系统限定的大小 ('+(upload_size/1024000).toFixed(2)+'MB 超过最大限制 '+lumise.data.max_upload_size +'MB)<br>请与管理员联系以更改服务器配置', 'error', 5000);
					// lumise.fn.notice('Error: your design is too large ('+(upload_size/1024000).toFixed(2)+'MB out of max '+lumise.data.max_upload_size +'MB)<br>Please contact the administrator to change the server configuration', 'error', 5000);
					return lumise.f(false);
				}
...
// alert(1);exit;
				...
				var donow = function() {
					try {
////////////////debugger;
						var data1 = JSON.parse(localStorage.getItem('LUMISE-CART-DATA'));
